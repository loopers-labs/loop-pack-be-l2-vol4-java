...
domain/<domain>/          - *Service(@Component), *Model(@Entity, BaseEntity 상속), *Repository(인터페이스)
...
- **`@Transactional` 은 domain Service 또는 Facade 에 유연하게 적용**. 단일 도메인 조작은 domain Service 에, 여러 도메인에 걸친 원자성이 필요한 경우(예: 주문 플로우)는 Facade 에 붙인다.
