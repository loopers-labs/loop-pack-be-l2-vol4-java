...
- 목록·페이징 조회는 `@RequestParam(defaultValue = ...) int page, int size`로 받아 Facade에 그대로 전달한다. page/size 범위 검증은 어느 계층에도 두지 않는다 — 클라이언트 값을 신뢰한다(`application/facade.md`).
- 정렬 등 쿼리파라미터를 도메인 타입으로 변환·검증하는 책임은 표현 계층에 있다. `@RequestParam(defaultValue = "latest") String sort`로 받아 `ProductSortType.from(sort)`로 도메인 enum을 만들어 Facade에 넘긴다. 허용되지 않는 값이면 `from()`이 `CoreException(BAD_REQUEST)`을 던지고 `ApiControllerAdvice`가 400으로 변환한다. 이는 비즈니스 검증이 아니라 입력 해석(parsing)이라 컨트롤러에 둔다.
...
// 관리자·페이징 — /api-admin 경로 + @RequestParam(page/size), 검증 없이 그대로 전달
@RestController
@RequiredArgsConstructor
...
// 정렬 쿼리파라미터 — Controller가 String을 도메인 enum으로 파싱해 Facade에 넘긴다
@Override
@GetMapping
public ApiResponse<ProductV1Dto.PageResponse> readProducts(
    ...
    Page<ProductSummaryInfo> productsSummaryInfo =
        productFacade.readProducts(brandId, ProductSortType.from(sort), page, size);
...
- ❌ 컨트롤러에 비즈니스 로직을 두지 않는다. 단, 쿼리파라미터를 도메인 타입으로 변환하는 입력 해석(`ProductSortType.from(sort)`)은 표현 계층 책임이다. page/size는 검증하지 않고 그대로 전달한다.
