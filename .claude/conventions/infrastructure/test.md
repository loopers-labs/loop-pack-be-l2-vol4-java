...
- 주입 대상은 도메인 `Repository` 인터페이스(`UserRepository`)다. `JpaRepository`를 직접 주입받지 않는다 — 계약(인터페이스) 레벨에서 검증한다.
...
## do / don't
- ✅ 도메인 `Repository` 인터페이스를 주입받아 계약 레벨에서 검증한다.
...
- ❌ `UserJpaRepository`를 직접 주입받지 않는다 — 도메인 Repository 인터페이스로 검증한다.
