...
sequenceDiagram
    ...
    OrderV1Controller->>OrderFacade: createOrder(userId, password, items)

    OrderFacade->>UserService: getUser(userId, password)
    UserService-->>OrderFacade: UserModel
    ...
    OrderV1Controller-->>Client: 201 Created
```
...
sequenceDiagram
    ...
    LikeV1Controller->>LikeFacade: addLike(userId, password, productId)

    LikeFacade->>UserService: getUser(userId, password)
    UserService-->>LikeFacade: UserModel
    ...
    LikeService->>LikeService: 중복 확인\n존재 시 409 Conflict
    LikeService-->>LikeFacade: LikeModel

    LikeFacade-->>LikeV1Controller: void
    LikeV1Controller-->>Client: 200 OK
```
...
sequenceDiagram
    ...
    BrandAdminV1Controller->>BrandAdminV1Controller: LDAP 헤더 검증\n불일치 시 403 Forbidden
    ...
    ProductService->>ProductService: 각 product.delete()

    BrandFacade-->>BrandAdminV1Controller: void
    BrandAdminV1Controller-->>Admin: 200 OK
```
