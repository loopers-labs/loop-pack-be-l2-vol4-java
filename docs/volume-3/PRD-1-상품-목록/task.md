...
- [X] T008 `ProductFacade.readProducts(brandId, sort, page, size)` sort 파싱+조회+map, `@Transactional(readOnly=true)` + 단위 테스트 — `application/product/ProductFacade.java`, `test/.../application/product/ProductFacadeTest.java`(ReadProducts: sort 무효 예외·정렬 위임)
...
- [X] T010 E2E — `test/.../interfaces/api/ProductV1ApiE2ETest.java`(ReadProducts: 200 메타·항목 키 / 정렬별 순서 / brandId 필터 / 빈 결과 200 / sort 무효 400)
