...
- [x] page/size: "미지정 시 기본"은 `@RequestParam(defaultValue)`. 범위 검증 없음(클라이언트 신뢰 방침).
...
### interfaces (편집)
- `BrandAdminV1Controller`: `@GetMapping` `readBrands(@RequestParam(defaultValue="0") int page, @RequestParam(defaultValue="20") int size)` → `brandFacade.readBrands(page, size)` → `ApiResponse.success(PageResponse.from(page))`.
...
### application
- `BrandFacade.readBrands(int page, int size)` (신규, `@Transactional(readOnly = true)`): `brandRepository.findActiveByPage(page, size)` → `.map(BrandInfo::from)` → `Page<BrandInfo>` 반환. (정렬은 쿼리 메서드명에 고정)
