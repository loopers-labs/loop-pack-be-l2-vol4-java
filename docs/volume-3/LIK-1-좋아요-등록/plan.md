...
회원이 `POST /api/v1/products/{productId}/likes`로 좋아요를 등록한다. Like 도메인의 첫 시나리오라 Like aggregate(`LikeModel` + `LikeRepository` + impl + JpaRepository)를 신설한다. `LikeFacade`가 회원 인증(`@LoginUser AuthenticatedUser`)으로 받은 userId와 path의 productId로, `UserRepository.getActiveById(userId)`로 회원 존재를 검증하고 `ProductRepository.getActiveById(productId)`로 상품 활성 여부를 검증한다(둘 다 없으면 NOT_FOUND). 이후 회원·상품 조합 존재 검사 후 없을 때만 저장한다(멱등). 회원 인증은 `AuthenticatedUserArgumentResolver`가 헤더로 처리한다.
...
- [x] `LikeModel` 생성: `@Builder`(`LikeModel.builder().userId(...).productId(...).build()`)
...
### application
- `application/like/LikeFacade.java` (신규) — `@Service @Transactional`. 주입: `UserRepository`, `ProductRepository`, `LikeRepository`. `createLike(Long userId, Long productId)` (void): `userRepository.getActiveById(userId)` → `productRepository.getActiveById(productId)` → `if (!likeRepository.existsByUserIdAndProductId(...)) likeRepository.save(LikeModel.builder()...build())`. 반환 없음(200 + 바디 없음).

### domain
- `domain/like/LikeModel.java` (신규) — `@Entity @Table(name="likes", uniqueConstraints=@UniqueConstraint(name="uk_likes_user_id_product_id", columnNames={"user_id","product_id"}))` extends `BaseEntity`. 필드: `@Column(name="user_id", nullable=false) Long userId`, `@Column(name="product_id", nullable=false) Long productId`. `@Builder` + `@NoArgsConstructor(PROTECTED)` + `@AllArgsConstructor(PROTECTED)`. VO 없음(필드 2개, 검증 규칙 없음).
...
| 결정 | 이유 | 기각한 더 단순한 대안 |
|------|------|----------------------|
| `LikeModel`에 VO 없이 `Long userId`·`productId` 직접 보유, `@Builder`로 생성 | 식별자 참조라 형식·길이 검증 규칙이 없음. `ProductModel.brandId` 선례와 동형 | `UserId`/`ProductId` VO 도입 — 검증할 불변식이 없어 빈 래퍼(과설계) |
...
| `LikeFacade`가 `UserRepository`·`ProductRepository` 직접 주입 | cross-domain 검증을 application이 타 도메인 Repository로 수행 — `ProductFacade`→`BrandRepository` 선례와 동형 | `LikeFacade`→`ProductFacade` 호출(facade 간 호출, 선례 없음) |
