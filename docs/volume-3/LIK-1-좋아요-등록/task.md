...
- [X] T006 `LikeFacade.createLike(userId, productId)` 작성 + 단위 테스트 — `main/.../application/like/LikeFacade.java`(`ProductRepository`·`LikeRepository` 주입; `!existsActiveById` → NOT_FOUND / 미등록 → save / 이미 등록 → no-op), `test/.../application/like/LikeFacadeTest.java`(CreateLike: 상품 미존재 NOT_FOUND / 미등록 시 save 호출 / 이미 등록 시 save 미호출)
