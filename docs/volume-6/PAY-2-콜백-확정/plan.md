...
### interfaces
- `interfaces/api/payment/PaymentV1Controller.java` (수정) — **`POST /api/v1/payments/callback`** 추가, `@ResponseStatus(OK)`, `handleCallback(@RequestBody CallbackRequest)` → 콜백 본문의 `orderId` 문자열을 `Long`으로, 처리 상태 문자열을 `PaymentStatus`로 매핑해 `PaymentFacade.handleCallback` 위임. 인증 없음
- `interfaces/api/payment/PaymentV1Dto.java` (수정) — `CallbackRequest(String orderId, PaymentStatus status, String reason)` 추가. pg-simulator가 보내는 전체 페이로드 중 **우리가 소비하는 필드만 바인딩**(나머지 `transactionKey/cardType/cardNo/amount`는 `FAIL_ON_UNKNOWN_PROPERTIES` disable로 무시 — `transactionKey`/`amount` 교차검증은 Stage 6에서 용도와 함께 추가). `status`는 `PaymentStatus` enum으로 받아 Jackson이 경계에서 string→enum 바인딩(미지원 값은 `READ_UNKNOWN_ENUM_VALUES_AS_NULL` disable로 400). 응답 본문 없음(`ApiResponse<Void>`)
- `interfaces/api/payment/PaymentV1ApiSpec.java` (수정) — `handleCallback` 오퍼레이션 추가

### application
- `application/payment/PaymentFacade.java` (수정) — `@Transactional handleCallback(Long orderId, PaymentStatus result, String reason)`:
  1. `paymentRepository.getByOrderId(orderId)` (없으면 NOT_FOUND)
  2. `payment.isTerminal()` 이면 무처리 반환(멱등)
  3. `payment.confirm(result, reason)` — 결제가 콜백 결과로 자기 전이를 캡슐화(SUCCESS→succeed/FAILED→fail/그 외 PENDING→무처리)
  4. 확정된 결제 상태로 주문 전이 결정: `payment.isSuccess()` → `orderRepository.getActiveById(orderId).markPaid()` / `payment.isFailed()` → `markPaymentFailed()`. 주문 전이 분기를 Order에 넣으면 Aggregate 간 타입 결합이 생기므로 응용 계층이 번역
  - 변경은 `@Transactional` 더티체킹으로 커밋 시 반영

### domain
- `domain/payment/PaymentRepository.java` (수정) — `PaymentModel getByOrderId(Long orderId)` 추가
- `domain/payment/PaymentModel.java` (수정) — 콜백 결과로 자기 전이를 캡슐화하는 `confirm(PaymentStatus result, String reason)` 추가, 주문 동반 전이 판단용 `isSuccess()`/`isFailed()` 추가(`isTerminal`은 이 둘로 재정의). PAY-1의 `succeed`/`fail` 재사용. 전이 단위 테스트를 본 cycle에서 보강
