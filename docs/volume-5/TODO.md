...
- [ ] 현재 코드(스칼라 서브쿼리 기반 like 카운트) **변경 없이** 측정
- [ ] EXPLAIN 계획 캡처 — S1~S4 (추정). 실측은 가능한 시나리오만(LIKES_DESC S1·S2 는 DNF 예상)
- [ ] k6 동시 50명 — S1~S4 p50/p95/p99 + 에러율 (30s 넘기면 DNF)
- [ ] **`reports/01-baseline.md` 작성** + `SUMMARY.md` 베이스라인 열 채움

**검증:** 베이스라인 보고서 완성, 가장 느린 시나리오(좋아요순) 식별.
