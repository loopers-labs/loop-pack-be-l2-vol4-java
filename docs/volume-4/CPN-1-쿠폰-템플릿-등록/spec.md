...
3. **Given** 관리자 인증 정보를 보유하고 최소 주문 금액을 생략했을 때, **When** 이름·타입·할인 값·만료 시각만으로 등록을 요청하면, **Then** 최소 주문 금액 0(제약 없음)으로 쿠폰 템플릿이 정상 생성된다.
...
6. **Given** 관리자 인증을 통과했을 때, **When** 최소 주문 금액을 지정했는데 0 미만으로 요청하면, **Then** 입력 검증 실패로 응답한다(400 BAD_REQUEST).
...
- 최소 주문 금액은 선택 입력이다. 지정하면 0 이상의 정수이며, 미지정 시 0(제약 없음)으로 저장하고 주문 금액 제약 없이 적용할 수 있다.
...
- **FR-005**: 시스템은 최소 주문 금액을 선택 입력으로 받아, 지정 시 0 이상인지 검증하고 미지정 시 0(제약 없음)으로 저장해야 한다. 0 미만이면 400으로 응답한다.
...
- **MinOrderAmount** (신규 VO): 0 이상 검증을 생성 시점에 단일화. 미지정(null) 시 0(제약 없음)으로 흡수해 항상 non-null.
- **ExpiredAt** (신규 VO): 만료 시각. null·기준 시각(`now`) 이전 금지를 `of(value, now)`에 단일화 (review 결정 — 사용자 입력값이라 VO로 캡슐화, `BirthDate`의 미래 날짜 금지와 대칭). 기준 시각은 `DateTimeUtil`로 표현 계층에서 주입.
...
| VO/Model 단위 | MinOrderAmount | 0 통과, null→0, -1 예외(errorType BAD_REQUEST) |
...
| VO/Model 단위 | Coupon | 생성 시 이름·타입·할인 값·최소 주문 금액(미지정 시 0)·만료 시각 보유, 정률 값 101·만료 시각 과거면 예외(errorType BAD_REQUEST) |
| Service/Facade 단위 | 쿠폰 템플릿 등록 유스케이스 | 정상 시 저장 후 식별자 반환, 할인 값 범위 밖이면 BAD_REQUEST·미저장 |
| Integration | CouponRepository | 저장·조회, 최소 주문 금액 0(미지정) 보존 |
