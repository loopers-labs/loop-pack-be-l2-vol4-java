...
- [ ] `ProductResponseDto` 수정 또는 신규 생성
  - 필드: `id`, `brandId` (또는 `brandName`), `name`, `price`, `likeCount`, `createdAt`
...
- [ ] `ProductQueryRepositoryImpl` 구현
  ...
  - 정렬 조건 분기 (`latest`, `price_asc`, `likes_desc`)
...
- [ ] `ProductFacade`에 파라미터 매핑 및 `@Transactional(readOnly = true)` 경계 적용
