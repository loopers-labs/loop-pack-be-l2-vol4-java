...
영속 메트릭과 실시간 랭킹의 장애 경계를 분리하고, Kafka 이벤트를 한국 날짜별 Redis ZSET에 가중 합산해 조회 전용 상품 랭킹을 제공한다. ZSET에는 음수와 0을 포함한 원점수를 저장하고 API에서만 `score > 0`을 노출한다. 종료된 날짜는 시간별 RDB 집계로 다시 계산한다.
...
주문 1건의 0.7점이 좋아요 3건의 0.6점보다 높다. Kafka 배치 안에서는 동일한 `(한국 날짜, 상품 ID)`의 점수를 먼저 합산해 Redis 호출 수를 줄인다. 여러 이벤트를 합친 결과가 0이면 0점 member를 남기기 위해 writer에 전달한다. 개별 점수가 0인 이벤트와 랭킹 대상이 아닌 이벤트는 집계에서 제외한다.
...
ranking consumer가 상품별 집계값을 Redis에 쓸 때마다 TTL을 2일로 다시 설정한다. 이벤트를 받았더라도 검증에서 제외되거나 실제 Redis 쓰기로 이어지지 않으면 TTL은 바뀌지 않는다. 읽기와 쓰기는 모두 master Redis를 사용해 갱신 직후 replica 지연으로 이전 순위를 읽는 문제를 피한다.
...
Redis 당일 랭킹은 유실 가능한 조회 모델이다. 별도 processed-event Set이나 Redis Inbox, Kafka 전체 replay 복구는 도입하지 않는다. 대신 종료된 날짜는 `product_metric_hourly`를 다시 합산한 스냅샷으로 교체한다.

이 선택에는 다음 한계가 있다.

- Redis 초기화 뒤 당일 랭킹은 새 이벤트부터 다시 점수가 쌓인다.
- 한 배치의 일부 Redis 쓰기 후 실패하면 offset이 acknowledge되지 않는다.
- 재전달 시 이미 성공했던 일부 점수가 중복 반영되어 소폭 오차가 생길 수 있다.

정확한 영속 통계와 종료일 재계산 기준은 `product_metric_hourly`가 담당한다. 당일에도 중복 없는 점수가 필요해지면 별도 Inbox나 멱등한 쓰기 방식을 비용과 함께 다시 검토한다.
...
- 동일 날짜/상품 배치 합산, 합산 결과 0의 writer 전달, 날짜가 다른 이벤트의 키 분리
- Consumer 성공 시 ACK, 역직렬화/의미 검증/처리 실패 시 ACK하지 않음
- 배치 전체 의미 검증을 쓰기 전에 완료해 poison event 앞의 점수가 부분 반영되지 않음
- Redis 원점수 누적, 실제 Redis 쓰기 시 2일 TTL 갱신, 음수·0 합계의 poll 경계 결정성
...
검증 당시 기존 조회 점수 `0.1`에 새 조회 이벤트가 반영되어 Redis 점수가 `0.2`가 되었고, `GET /api/v1/rankings?date=20260717&page=1&size=20` 응답에서도 `rank=1`, `score=0.2`를 확인했다. 이 검증은 실제 브로커를 사용한 수동 실시간 smoke이며, 자동화 테스트의 구성 요소별 검증과 구분한다.
...
현재 이벤트 기반 구현은 유지하면서 `product_metric_hourly`를 읽어 종료된 날짜의 일간 원점수를 다시 발행하는 배치 스냅샷을 추가했다. 구현 구조, 10만 건/10만 상품 측정, 재실행 정확성과 신선도 경계는 [이벤트 랭킹과 배치 스냅샷 비교](04-event-vs-batch-ranking.md)에 기록했다. 기존 벤치마크 결론을 대체하는 것이 아니라 당일 이벤트 처리와 종료일 보정의 역할 차이를 검증한 확장이다.
