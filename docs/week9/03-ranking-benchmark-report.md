...
다만 batch 3,000의 75%처럼 많은 상품이 비슷한 방향으로 과대 계상되면 점수값은 크게 틀려도 Top 20 구성은 유지될 수 있다. 반대로 batch 500의 75%처럼 상위권 일부가 선택적으로 중복되면 더 작은 총점 오차로도 Top 20 구성이 크게 바뀔 수 있다. 여기서 일치율은 순서를 무시한 집합 overlap이므로 순위 순서 보존까지 증명하지 않는다. 운영 관측에서는 총점 오차 대신 재처리 횟수, 영향 member 수, Top-N overlap과 순서 변동을 함께 봐야 한다.
