...
| `stock` | ✅ VO (`Stock`) | "0 이상" 불변식 + `deduct`/`restore`/`isAvailable` 행동을 가진 값. 불변(연산 시 새 `Stock` 반환). `@Embeddable`로 `product.stock` 컬럼에 매핑. `Product.deductStock`은 위임 |
| `likesCount` | ❌ primitive | 비정규화 카운터(04 §4.2 reconcile 대상). "음수 방지"는 drift 가드일 뿐 도메인 불변식 아님 — 진짜 값은 `COUNT(product_like)` |
