...
*   **주문 생성 멱등성 (재시도 및 동시성 방어):** 
    ...
    *   Redis의 `SETNX`(분산 락)를 활용해 동시성(Race Condition)을 제어하며, 사용자 간 충돌을 막기 위해 **키를 유저 ID와 함께 네임스페이스(`order:create:{userId}:{idempotencyKey}`)로 분리**한다.
...
*   **결제 중복 방지 및 실패 보정:**
    ...
    *   **조회부터 결제 내역 저장까지의 과정을 Redis 분산 락(`payment:lock:{orderId}`)으로 묶어** 원자성(Atomicity)을 보장함으로써, 동시 요청 시 중복 활성 결제가 생성되는 것을 원천 차단한다.
