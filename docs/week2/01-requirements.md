...
| METHOD | URI | user_required | 설명 |
| --- | --- | --- | --- |
| POST | `/api/v1/orders/checkout` | O | 주문 및 결제 동시 요청 (단일 API, 내부적으로 트랜잭션 분리 및 PG사 연동 처리) |
...
- **트랜잭션 분리 및 단일 API (주문 & 결제):** 클라이언트 요청 편의를 위해 주문 생성과 결제를 단일 API(`/api/v1/orders/checkout`)로 제공하지만, 서버 내부(Facade)에서는 락(Lock) 점유 및 커넥션 풀 고갈을 막기 위해 트랜잭션을 분리한다. (T1: 주문 생성 및 재고 가선점 완료 -> PG API 외부 호출 -> T2: 결제 이력 저장 및 상태 업데이트. 실패 시 예외를 잡아 보상 트랜잭션 수행)
...
- [x] 상품 상세 및 목록 조회 시, COUNT 쿼리를 통해 동적으로 계산된 좋아요 수가 정확히 제공되는가?
...
- [ ] 좋아요 등록/취소 시 비관적 락 없이 `PRODUCT_LIKES` 데이터만 안전하게 적재/삭제되는지 검증하는가? (동시성 제어 불필요)
