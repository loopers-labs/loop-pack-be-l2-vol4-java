...
*   **결제 응답 지연 및 유실 보정 (연쇄 Redis TTL):** PG사의 콜백 응답이 지연되거나 유실되는 경우를 대비해, 결제 요청 시 Redis에 재시도 횟수(초기값 0)와 함께 10초 TTL 키를 생성한다.
...
*   **최후의 보루 (Fallback Batch):** Redis 서버 장애로 인해 TTL 이벤트가 유실되어 DB에 장기간 `PENDING` 상태로 남는 결제 건을 보정하기 위해, 주기적(예: 매일 새벽)으로 스케줄러를 실행하여 누락된 건들에 대해 상태 확인 및 보상 트랜잭션을 수행한다.
