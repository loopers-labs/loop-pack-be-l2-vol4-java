...
> 개발을 모르는 사람도 이해할 수 있는 수준으로 작성한 다이어그램입니다.
> 구현 세부사항 없이 **"누가 무엇을 요청하고, 시스템이 어떻게 반응하는가"** 에 집중합니다.
...
sequenceDiagram
    ...
    alt 존재하지 않는 상품인 경우
        서비스-->>사용자: 상품을 찾을 수 없습니다
    else 이미 좋아요한 상품인 경우
        서비스-->>사용자: 이미 좋아요한 상품입니다
    else 정상
        서비스-->>사용자: 좋아요가 등록되었습니다
    end
```
...
sequenceDiagram
    ...
    사용자->>서비스: 상품 주문 요청 (여러 상품, 수량 포함)

    loop 주문한 상품 각각 확인
        alt 존재하지 않는 상품인 경우
            서비스-->>사용자: 존재하지 않는 상품이 포함되어 있습니다
        else 재고가 부족한 경우
            서비스-->>사용자: 재고가 부족합니다
        end
    end

    Note over 서비스: 재고 차감 및 주문 기록 저장<br/>(주문 당시 상품명·가격도 함께 기록)
...
sequenceDiagram
    ...
    alt 존재하지 않는 브랜드인 경우
        서비스-->>어드민: 브랜드를 찾을 수 없습니다
    else 정상
        Note over 서비스: 해당 브랜드의 상품들도 함께 삭제 처리
        서비스-->>어드민: 브랜드가 삭제되었습니다
    end
```
...
> 레이어별 책임 분리, 트랜잭션 범위, 예외 처리 흐름을 확인하기 위한 개발자용 다이어그램입니다.
