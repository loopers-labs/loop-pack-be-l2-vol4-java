...
### 주문 대기열 — Redis 키 (Round 8)

대기열은 **MySQL 테이블을 추가하지 않는다.** 대기 순서·입장 토큰은 Redis 자료구조로만 존재한다 — 스키마는 없지만 데이터 설계이므로 이 문서에 함께 기록한다.

| Key | 자료구조 | 역할 | TTL |
|------|------|------|------|
| `waiting-queue` | Sorted Set (score=진입 시각 ms, member=userId) | 대기 순서 보관. Set 특성으로 중복 진입 자동 방지, `ZRANK` O(log N) 순번 조회 | 없음 (스케줄러 `ZPOPMIN` 이 소비) |
| `entry-token:{userId}` | String (UUID) | 주문 입장 권한 증명. 주문 성공(2xx) 시 DEL 로 소진, 실패 시 보존 | 300s (설정값) |
| `processing-token:{userId}` | String (owner token) | 유저당 in-flight 주문 마크. `SETNX` 로 선점해 동시 주문을 1건으로 제한(2건째부터 409), 완료 시 값 일치할 때만 DEL(compare-and-delete) | 30s (크래시 자동 해제 안전망) |

- **왜 RDB 가 아닌가** — 대기열은 폭증 트래픽의 **완충 지대**다. 진입을 DB INSERT 로 받으면 보호하려는 커넥션 풀을 보호 장치가 소모한다(모순). 순번 조회는 `ZRANK` 로 대기 인원과 무관하게 싸고, 토큰 만료는 TTL 이 자연 처리해 만료 배치·상태 컬럼이 필요 없다. 데이터가 행사 종료와 함께 버려지는 휘발성이라 내구성 요구도 없다.
- **진입 멱등 = `ZADD NX`** — 재진입 시 score(진입 시각)를 갱신하지 않아 순번이 보존된다. 단순 ZADD 면 대기 화면 새로고침(=재진입)마다 줄 맨 뒤로 리셋된다.
- **동시 주문 마크를 토큰과 분리한 이유** — 토큰(`entry-token`)은 "입장 권한"이고 성공 시에만 소진(실패 보존)이라, 여기에 "처리 중" 상태를 겸하게 하면 실패 시 보존과 충돌한다. 별도 `processing-token` 을 `SETNX EX` 로 선점(선점·TTL 원자화)해 in-flight 배타를 표현하면, 토큰은 소진 규칙만 지키고 마크는 동시성만 막아 두 관심사가 독립적이다. 오버셀 자체는 재고 핫로우 비관락이 이미 막으므로, 이 마크는 정확성이 아니라 공정성·하류 증폭 방지가 목적이다.
- **RDB 유니크 제약의 등가물** — `user_coupons (user_id, template_id)` 유니크가 1인 1매를 지키듯, 대기열의 "1인 1자리"는 ZSET 의 member 유일성이 지킨다. 스키마 제약이 자료구조 성질로 옮겨간 것이다.
- **일관성 라우팅** — 대기열·토큰 연산은 **master 로 고정**한다. 기본 커넥션은 replica 우선이라 `ZADD` 직후 `ZRANK` 가 복제 지연으로 nil 을 반환할 수 있다 — 캐시와 달리 대기열은 그 자체가 진실의 원천(source of truth)이므로 읽기 분산의 이득보다 정합성이 우선이다.
- **토큰 TTL 300s 의 근거** — 토큰 수신 인지(폴링 주기 최대 수 초) + 주문서 작성 체류 시간. 너무 짧으면 정상 유저가 만료로 튕겨 재진입(불공정 + 부하 재생산), 너무 길면 이탈 유저가 발급 슬롯을 오래 점유해 뒤 유저의 대기가 늘어난다. 만료율을 보고 조정하는 운영 파라미터로 두고 시작값만 정했다.
