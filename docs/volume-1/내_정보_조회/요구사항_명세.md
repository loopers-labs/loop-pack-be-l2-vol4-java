...
본인 인증은 커스텀 `@LoginUser` 어노테이션과 `AuthenticatedUserArgumentResolver`로 처리한다. Resolver가 요청 헤더에서 `X-Loopers-LoginId`/`X-Loopers-LoginPw`를 추출해 회원을 조회하고 `UserModel.authenticate(...)`로 본인 인증을 수행한 뒤, 컨트롤러에 `AuthenticatedUser`(인증된 회원의 `userId`만 보유한 경량 record)를 주입한다. 컨트롤러는 엔티티에 의존하지 않고 `userId`만으로 후속 조회를 진행한다. 인증이 필요한 모든 엔드포인트에서 동일하게 재사용한다.
...
sequenceDiagram
    ...
    Resolver->>Domain: matchesPassword(rawPassword, passwordEncrypter)
    Domain-->>Resolver: true
    Resolver-->>Controller: @LoginUser AuthenticatedUser(userId)
    Controller->>Facade: readMyInfo(userId)
    Facade->>Service: readMyInfo(userId)
    Service->>Repository: findById(userId)
    Repository-->>Service: UserModel
    Service-->>Facade: UserModel
    Facade-->>Controller: UserMyInfo (name = name.maskedValue())
...
본 라운드는 회원 도메인의 **조회 + 비밀번호 일치 여부 확인** 행위를 추가한다. 새로운 도메인 객체나 값 객체는 도입하지 않고, 기존 객체에 행위만 보강한다.

| 역할 (객체명) | 책임 | 속성 | 행위 (본 라운드 추가는 **굵게**) |
|---|---|---|---|
| **`UserModel`** *(JPA Entity)* | 가입한 회원 한 명을 표현. 자기 자신의 비밀번호 일치 여부 확인 책임을 가진다. | `id` (`BaseEntity`로부터), `loginId: LoginId`, `encryptedPassword: EncryptedPassword`, `name: Name`, `birthDate: BirthDate`, `email: Email` | 기존: `new UserModel(...)`. **추가: `matchesPassword(rawPassword: String, passwordEncrypter: PasswordEncrypter)` → `boolean` — 자신의 `encryptedPassword.matches(...)`에 위임** |
