...
- [ ] `domain/queue/WaitingQueueRepository` 인터페이스 (`enter`, `rank`, `size`)
- [ ] `infrastructure/queue/WaitingQueueRepositoryImpl` (기본/Master 템플릿 연산별 분기)
- [ ] `domain/queue/EntryTokenRepository` 인터페이스 (`find`, `delete`)
- [ ] `infrastructure/queue/EntryTokenRepositoryImpl`
- [ ] Testcontainers Redis로 `ZADD`/`ZRANK`/`ZCARD`/`GET`/`DEL` 동작 통합 테스트
...
- [ ] `resources/scripts/admit-batch.lua` 작성
- [ ] `domain/queue/QueueAdmissionRepository` 인터페이스 (`admitBatch`)
- [ ] `infrastructure/queue/QueueAdmissionRepositoryImpl` (`DefaultRedisScript`)
- [ ] 통합 테스트: N명 pop + 토큰 발급 원자성, "대기열에도 없고 토큰도 없는" 중간 상태가 관측되지 않는지 검증
...
- [ ] `WaitingQueueService` — Repository mock 단위 테스트
- [ ] `EntryTokenService` — Repository mock 단위 테스트
...
- [ ] `QueueProperties`(`@ConfigurationProperties(prefix = "queue")`) 작성
- [ ] `application.yml`에 `queue.scheduler-interval-ms`, `queue.throughput-per-second` 추가
...
- [ ] 스케줄러 컴포넌트(`@Scheduled(fixedDelayString = "${queue.scheduler-interval-ms}")`) 작성, `QueueAdmissionRepository.admitBatch(queueProperties.batchSize(), ttl)` 호출
- [ ] `spring.task.scheduling.pool.size` 최소 3~4로 조정 (기존 `OutboxRelay`/`PaymentReconciliationScheduler`와의 스레드풀 경합 예방)
- [ ] 헬스체크 gauge 코드 추가(`queue.scheduler.last.execution.timestamp`, 매 tick 종료 시 `AtomicLong` 갱신 + `MeterRegistry.gauge(...)` 등록)
...
- [ ] `QueueFacade`(`enter`, `getPosition`) + `QueueInfo` — 통합 테스트(Testcontainers)
- [ ] `QueueV1Controller`, `QueueV1Dto`, `QueueV1ApiSpec` — E2E 테스트
- [ ] `http/commerce-api/queue.http` 등으로 수동 검증: `enter` → `position` polling → 스케줄러가 토큰 발급하는 것까지 눈으로 확인
...
- [ ] `ErrorType.SERVICE_UNAVAILABLE`(503) 추가
- [ ] `ApiControllerAdvice`에 Redis 연결 예외(`DataAccessException` 계열) 전용 핸들러 추가
- [ ] `OrderFacade`에 `EntryTokenService` 의존 추가 — 진입 전 `verify`, 성공 후 `consume`(`delete`)
- [ ] `OrderV1Controller`에 `X-Loopers-EntryToken` 헤더 추가
- [ ] E2E: `enter` → `position` polling → 토큰 발급 → `POST /orders` 전체 플로우, 토큰 없음/만료 시 403 검증
...
- [ ] Resilience4j Circuit Breaker 도입 (Redis 호출부 감싸기)
- [ ] 주문 API 자체 Rate Limit (문턱값 150 TPS)
- [ ] Prometheus 알림 규칙("스케줄러 마지막 성공 실행으로부터 5~10초 경과") + Grafana 대시보드
