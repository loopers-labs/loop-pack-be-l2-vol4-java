...
대기열의 목적은 처리량을 하류가 감당하는 선으로 누르는 것이다. 그래서 부하 테스트의 1차 목표는 통과/실패가 아니라 **숫자를 재는 것** — 주문 API가 무너지기 시작하는 지점(무릎)을 찾아 `batch-size`를 정한다. 스크립트 본체는 `docs/k6/`에 두고, 이 문서는 설계 의도·실행법·결과 기록 틀이다.
...
1. (Before) 큐 없이 주문 API 맨몸에 부하를 램프업
   ...
   → p99 지연이 꺾이거나 풀이 포화되는 지점 = 안전 TPS(무릎)
...
| ID | 분류 | 검증 | 스크립트 |
| --- | --- | --- | --- |
| **T1** | 처리량 무릎 | 큐 우회, 주문 API 직접 램프업 → p99·HikariCP active로 안전 TPS 도출 | `order-knee.js` |
...
| 지표 | 소스 | 보는 이유 |
| --- | --- | --- |
| 주문 처리시간 p50/p95/p99 | Micrometer `Timer` | 무릎(지연 급증) 탐지 |
...
| 실행일 | interval-ms | batch-size | 안전 TPS(무릎) | p99 | 비고 |
| --- | --- | --- | --- | --- | --- |
| | | | | | |

> 측정 후 `01-requirements.md`의 175/35 초기값을 실측치로 갱신하고, 그 근거(무릎이 나온 지점·지표)를 이 표에 남긴다.
