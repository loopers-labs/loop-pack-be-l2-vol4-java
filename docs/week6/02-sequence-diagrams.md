...
호출 순서가 아니라 **트랜잭션 경계와 상태 전이**를 검증한다. 특히 PG 호출이 어떤 트랜잭션에도 들어가지 않는 점, 콜백·정합성 보정의 멱등 전이, failover의 안전 트리거가 핵심이다.
...
2. **TX1에서 PENDING을 먼저 커밋**해 "결제 시도" 의도를 남긴다. PG 호출 전 크래시는 PG가 모르므로(돈 안 움직임) 안전, TX2 전 크래시는 주문 기준 보정(orderId)으로 복구.
3. 멱등은 `order_number` 유니크가 1차 방어선이다. 사전 `exists` 체크는 사용자 메시지용 보조.
...
2. failover는 **"안 나간 게 확실한 예외"에만**. 보내진 뒤 모호한 실패는 정합성 보정으로 — A가 이미 처리했을 수 있어 B로 보내면 이중 결제.
...
sequenceDiagram
    ...
    note right of PR: 없으면 무시(또는 정합성 보정 대기)
...
**설계 의도**:
1. **transactionKey 단독 조회**. "키 저장 전 콜백 도착" 레이스는 콜백 지연(1~5초) ≫ TX2(ms)라 사실상 발생하지 않고, 발생해도 키 기준 보정이 회수하므로 핸들러를 단순하게 둔다.
2. **`PENDING`일 때만 전이**(멱등 가드). 콜백과 정합성 보정이 같은 Payment를 동시에 건드려도 1회만 반영한다(+ 낙관적 락).
3. **FAILED는 단일 트랜잭션** — 상태 전이 + 보상을 한 TX로 묶는다. 부분 실패 시 통째 롤백 → Payment PENDING 유지 → 정합성 보정이 재처리(콜백 무보증이라 재전송 없음).
4. 콜백엔 서명/인증이 없으므로 `orderId·amount` 일치를 검증한 뒤에만 전이한다.

## 4. 정합성 보정 — @Scheduled

콜백 유실(키 보유)과 TX2 전 크래시(키 없음)를 두 정합성 보정(키 기준·주문 기준)으로 회수하는지 검증한다.
...
sequenceDiagram
    ...
    note over Rec,PG: 키 기준 보정 — 콜백 유실
    ...
    note over Rec,PG: 주문 기준 보정 — TX2 전 크래시
    Rec->>PR: PENDING + key IS NULL + N초 경과
    loop 각 Payment
        Rec->>PG: GET /payments?orderId=
        alt PG에 거래 있음
            Rec->>PR: 키 채움 → 키 기준 보정 경로로 확정
        else 거래 없음
            Rec->>H: FAILED 정리(+보상)
        end
    end
...
2. **FAILED 처리는 콜백과 공유 진입점**(`PaymentResultHandler`)을 호출한다. 콜백이 단일 TX 롤백으로 못 끝낸 보상을 정합성 보정이 동일 로직으로 마저 수행한다. 보상 로직이 두 군데로 갈라지지 않게 한다.
...
3. **정합성 보정 주기 vs UX**: 콜백 유실분은 정합성 보정 주기만큼 확정이 지연된다. 사용자에겐 "결제 처리 중" 상태를 노출하고, 최종 결과는 주문 상세/알림으로 전달한다(동기 응답으로 내려주지 않는다).
