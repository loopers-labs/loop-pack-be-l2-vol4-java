...
**설계 의도**:
- `Payment`는 **멱등 전이의 불변식**을 캡슐화한다. `markSuccess()`/`markFailed()`는 `PENDING`에서만 허용하고(아니면 무시/예외), 콜백·대사가 같은 객체를 동시에 건드려도 1회만 전이된다.
- `Order`도 **전이를 자기 메서드로 캡슐화**한다. `markPaid()`/`markPaymentFailed()`는 `PENDING_PAYMENT`에서만 동작하고 이미 terminal이면 무시한다 — `PaymentResultHandler`가 `order.setStatus(...)`로 규칙을 밖으로 빼지 않게 한다(Payment의 `markSuccess/markFailed`와 대칭). 콜백·대사가 같은 주문을 동시에 확정해도 멱등이다. (현재 week4 `Order`는 가드 없는 `markFailed()`만 있어, week6 구현 시 이 형태로 정렬한다.)
...
classDiagram
    ...
    class PaymentResultHandler {
        +handle(transactionKey, PaymentResult) void
    }
    ...
    PaymentResultHandler --> OrderRepository
    PaymentResultHandler --> OrderCompensationService
...
**설계 의도**:
1. **`PaymentService.pay`** — TX1(PENDING 생성) → PG 호출(트랜잭션 밖) → TX2(키 확정)의 3구간을 조율한다. 트랜잭션 경계를 가르는 책임이라 `@Transactional`을 메서드 전체에 걸지 않는다(내부에서 구간별로 분리).
2. **`PaymentResultHandler`가 콜백·대사의 공유 진입점**이다. SUCCESS/FAILED 확정과 보상을 한곳에 모아, 콜백과 대사가 같은 멱등 로직을 호출하게 한다. 보상 분기가 갈라지는 버그를 막는다. `OrderRepository`를 직접 부르지만 상태 전이는 `order.markPaid()`/`order.markPaymentFailed()` 도메인 메서드에 위임한다 — Handler는 로드·조율만 하고 전이 규칙(가드)은 `Order` 안에 둔다.
3. **`PaymentReconciler`**는 `@Scheduled`로 두 sweep을 돌리고, 확정은 `PaymentResultHandler`에 위임한다(자체 보상 로직을 갖지 않는다).
4. `OrderCompensationService`는 `docs/week4`의 재고·쿠폰 복구를 재사용한다(결제 실패 보상의 실체).
