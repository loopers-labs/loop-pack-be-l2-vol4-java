...
- `throwsAlreadyExists_whenDuplicateLoginIdViolatesDatabaseConstraint`
  - DB unique 제약에서 중복 로그인 ID가 감지되어도 `USER_ALREADY_EXISTS` 예외로 변환되는지 검증한다.
- `returnsMyInfo_whenLoginHeadersAreValid`
...
- DB unique 제약에서 발생한 중복 로그인 ID 예외도 `USER_ALREADY_EXISTS` 에러 타입으로 변환한다.
