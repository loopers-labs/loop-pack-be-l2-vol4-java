...
이 문서는 현재 4주차 구현의 테스트 보조 문서다. 제출 커밋에는 포함하지 않는다.
...
| 순위 | 영역 | 이유 |
| --- | --- | --- |
| 1 | 주문 생성, 재고 차감, 쿠폰 사용 | 초과 판매와 쿠폰 중복 사용을 막는 핵심 경계다. |
| 2 | 쿠폰 발급 | 사용자별 최대 발급 횟수를 동시 요청에서도 지켜야 한다. |
| 3 | 주문 실패 롤백 | 재고 또는 쿠폰 처리 실패 시 주문 관련 변경이 남지 않아야 한다. |

## 기존 확장 검증

아래 항목은 유지하지만 4주차 필수 검증과 분리한다.

| 순위 | 영역 | 이유 |
| --- | --- | --- |
| 1 | 결제 성공/실패/타임아웃 | 주문 상태, 재고, 쿠폰 상태 불일치를 막아야 한다. |
| 2 | 좋아요 등록/취소 | 멱등성과 카운터 정합성이 중요하다. |
| 3 | Outbox 전송과 조회 API | 완료 이벤트와 응답 shape가 설계와 맞아야 한다. |
