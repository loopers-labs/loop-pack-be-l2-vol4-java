...
sequenceDiagram
    ...
    유저->>서비스: 회원가입 (아이디, 비밀번호, 이름, 생년월일, 이메일)
    activate 서비스
    ...
    서비스-->>유저: 가입 완료
    deactivate 서비스
    Note over 서비스: 웰컴 쿠폰 자동 발급 예정 (기능 확장)<br/>이메일은 향후 알림 채널의 도달 주소
```
...
sequenceDiagram
    ...
    Note over 유저,서비스: 주문 완료 후 결제 페이지로 진입 (주문번호 + 결제수단 선택)
    유저->>서비스: 결제 요청 (주문번호, 결제수단, 금액)
    activate 서비스

    alt 타인의 주문에 결제 시도
        서비스-->>유저: 실패 — 권한 없음
    else 이미 결제된 주문 (CONFIRMED) 또는 취소된 주문 (CANCELLED)
        서비스-->>유저: 실패 — 결제 불가 상태
    else 결제 금액이 주문 금액과 불일치
        서비스-->>유저: 실패 — 금액 불일치
    end

    Note over 서비스,PG: 외부 PG 호출 — 응답 대기

    서비스->>PG: 결제 요청 (결제수단, 금액)
    activate PG

    alt PG 결제 성공
        PG-->>서비스: 성공 (거래번호)
        Note over 서비스: 주문 상태 → 확정(CONFIRMED)<br/>결제 정보 저장
        서비스-->>유저: 결제 완료 (주문번호, 결제 금액, 거래번호)
    else PG 결제 실패
        PG-->>서비스: 실패 (실패 사유)
        Note over 서비스: 주문 상태 → 취소(CANCELLED)<br/>차감된 재고 자동 복구
        서비스-->>유저: 실패 — 결제 실패 (사유 안내)
    end
    deactivate PG
    deactivate 서비스
```
...
> **결제 정책 요약 (기획 시 참고):**
> - 한 주문에 결제는 **1회만** 시도 가능 (MVP). 분할·다중 수단은 향후 확장
> - 결제 실패 시 **재고 자동 복구**. 유저는 재주문해야 함 (현재 재시도 UX 없음)
> - PG 응답 지연·타임아웃 시 처리 정책 미정 — 운영 데이터 수집 후 결정
> - 쿠폰 적용은 향후 확장: 적용 시점·복구 정책은 쿠폰 기획안에서 정의

---

### 5-2. 결제 실패 후 재시도 UX

> **현재 MVP에는 없음.** 결제 실패 시 주문이 자동 취소되어 유저가 처음부터 다시 주문해야 한다.
> 향후 다음 옵션을 검토:
>
> | 옵션 | 설명 | 트레이드오프 |
> |------|------|-------------|
> | A. 주문 보존 + 결제 재시도 | 실패 후 일정 시간 동안 PENDING 유지, 재결제 허용 | 재고 점유 시간 증가 |
> | B. 즉시 취소 + 빠른 재주문 도우미 | 동일 상품을 빠르게 다시 담기 | 기술 단순, UX 마찰 |
