...
erDiagram
    ...
    payments {
        bigint id PK
        bigint order_id UK "NOT NULL, 논리적 FK, 중복 결제 방지"
        varchar(50) transaction_key "PG 발급 키, nullable (콜백 수신 후 저장)"
        varchar(10) card_type "SAMSUNG|KB|HYUNDAI, NOT NULL"
        varchar(19) card_no "xxxx-xxxx-xxxx-xxxx, NOT NULL"
        bigint amount "NOT NULL"
        varchar(10) status "PENDING|SUCCESS|FAILED, NOT NULL"
        varchar(200) failure_reason "nullable, FAILED 시 PG 사유"
        datetime created_at "NOT NULL"
        datetime updated_at "NOT NULL"
    }
...
| payments | order_id | 동일 주문 중복 결제 방지 |
...
| payments | (status, created_at) | PENDING 복구 배치 조회 (`status=PENDING AND created_at < ?`) |
...
| payments | Soft Delete 없음 | 결제 이력 영구 보존, status로 상태 관리 |
