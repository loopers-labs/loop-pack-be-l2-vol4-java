...
> `members.login_id`, `members.email` Unique 제약은 활성 계정(`deleted_at IS NULL`)에만 적용합니다.
> MySQL은 Filtered Index(`WHERE` 조건부)를 지원하지 않으므로 Generated Column으로 구현합니다.
>
> ```sql
> ALTER TABLE members
>   ADD COLUMN email_unique_key VARCHAR(100)
...
## 잠재 리스크
- `members.login_id`, `members.email` Unique 제약은 활성 계정에만 적용 → 탈퇴 후 동일 이메일/로그인 ID 재가입 가능 (Generated Column으로 구현)
