...
| D7 | **다도메인 협력은 Facade `@Transactional` 합성 (B안)**. 주문은 `OrderFacade`가 `ProductService` + `StockService` + `OrderService`를 *분기 없이* 합성. 좋아요는 `LikeFacade`가 `LikeService`(멱등 등록/취소 — 실제 반영 여부를 boolean으로 반환) + `ProductService`(`like_count` 증감)를 합성하며, *실제 반영된 경우에만* 카운터를 증감한다 | 주문은 분기 없는 합성이라 규약 그대로. 좋아요는 `Service↔Service 금지` 규약상 `LikeService`가 `ProductService`를 직접 호출할 수 없어 합성 자리가 `LikeFacade`뿐이다. 멱등 반영 여부(boolean)에 따라 카운터 증감을 결정해야 하므로 `LikeFacade`에 *최소 분기*가 생기는데, 이는 `Service↔Service 금지`와 `Facade 분기 금지`가 충돌하는 지점에서 전자를 우선한 결과다. `like_count`는 약한 일관성(D3)이고 트랜잭션 내 외부 I/O가 없어 Facade 합성으로 충분하다. 본 라운드 한정 |
