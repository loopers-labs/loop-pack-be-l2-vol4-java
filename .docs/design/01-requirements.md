...
### 1.3 시스템 관점
다상품 주문 시 ① 재고 정합성, ② 주문 당시 상품 정보 스냅샷, ③ (향후) 포인트·외부 PG 차감의 원자성이 필요하다.
...
- 인증·인가는 구현하지 않는다. 헤더 기반 식별만 사용.
- 유저는 타 유저의 좋아요·주문에 직접 접근할 수 없다.
- 어드민(`/api-admin/v1`)은 이번 라운드 **scope out**.
...
| Brand | 신규 | 상품의 소속 기준. 대고객은 단건 조회만 |
| Product | 기존 확장 | `brandId` 참조 + `likeCount` 비정규화 컬럼. 재고는 보유하지 않음 |
...
이번 라운드 **시퀀스 다이어그램에는 등장**하지만, 도메인 세부 구조(모델·테이블·잔액 정책 등)는 정의하지 않는다. *흐름 단계*만 합의하고 구현은 다음 라운드.

| 대상 | 형태 | 본 라운드 표현 범위 |
|---|---|---|
| Payment | 도메인 (세부 미정의) | **포인트·PG 통합 책임자**. 주문 도메인은 결제 한 군데만 호출 |
| Point | 도메인 (세부 미정의) | **결제 도메인이 호출**. 주문이 직접 호출하지 않음 |
...
| 기능 | API | 설명 |
|---|---|---|
| 브랜드 단건 조회 | `GET /api/v1/brands/{brandId}` | 브랜드 기본 정보 응답 |

**유저 시나리오**
- 유저는 브랜드 ID로 브랜드 정보를 조회할 수 있다.
- 존재하지 않거나 soft delete된 브랜드 → `404 NOT_FOUND`.
...
| 기능 | API | 설명 |
|---|---|---|
| 상품 목록 조회 | `GET /api/v1/products` | 필터·정렬·페이지네이션 |
...
**유저 시나리오**
- 유저는 상품 목록을 페이지 단위로 조회할 수 있다.
- 브랜드 필터로 특정 브랜드의 상품만 본다.
- 최신순(기본) / 가격 오름차순 / 좋아요 내림차순 정렬을 선택할 수 있다.
- 삭제된 상품은 목록·단건 모두 노출되지 않는다.
- 재고가 0인 상품은 노출한다 — 품절 표기는 클라이언트 책임.
...
- 좋아요 수는 `COUNT(product_like)` 집계로 구한다 (비정규화 컬럼 없음). `likes_desc` 정렬도 COUNT 기반.
...
| 주문 목록 조회 | `GET /api/v1/orders?startAt=&endAt=` | 본인 주문, 날짜 범위 |
| 주문 단건 조회 | `GET /api/v1/orders/{orderId}` | 본인 주문만 |
...
- 결제는 이번 라운드 미구현 — Order는 생성 즉시 `CREATED` 상태로 종료.
...
3. 결제 도메인 내부에서:
   - **포인트 차감** — `PointService.deduct(userId, usedPoint)`
   - **외부 PG 호출** — `PaymentGatewayClient.request(orderId, pgAmount)`
   - **Payment record 저장** — `usedPoint + pgAmount + totalAmount`를 *지불 사실의 단일 진실 원천*으로 보존
4. 결과에 따른 분기
   - 성공 → `SUCCEEDED`
   - 실패 → 결제 도메인이 *자기 영역* 보상(포인트 환원 + PG 취소). 그 후 주문 도메인이 *상품 도메인에게* 재고 복원 요청. **재고 자원은 상품 도메인 단일 책임이며, 다른 도메인은 직접 만지지 않는다.**
...
| API prefix | 대고객 `/api/v1`. 어드민 `/api-admin/v1` scope out |
...
| # | 결정 | 사유 / 영향 |
|---|---|---|
| D1 | 결제 흐름을 **시퀀스 레벨에서만 표현**. Payment/Point 도메인의 세부 모델·테이블·정책은 정의하지 않음 | Quest 원문 "결제 흐름 (재고 차감, 포인트 차감, 외부 시스템 연동)" 충실 + brainstorming 비용 절제. 구현은 다음 라운드. *흐름 단계*가 시퀀스에 그려져 있으면 다음 라운드 진입 시 시퀀스 재작성 불필요 |
| D2 | OrderItem 스냅샷 = `productId, productName, unitPrice, quantity` 4개 최소 | 영수증 수준의 완전 복사는 과잉. 브랜드명·이미지는 조회 시 fallback |
| D3 | 좋아요 수는 `COUNT(product_like)` 집계 (비정규화 컬럼 미사용) | 동기화 부담·hot row 경합 회피 + 항상 정확. `likes_desc` 정렬은 COUNT GROUP BY/서브쿼리에 의존 (상품 대량 시 성능 부담은 다음 라운드 캐싱으로 진화) |
| D4 | `OrderStatus` enum 도입. 값: `CREATED` → `SUCCEEDED` / `FAILED` | 시퀀스에 그려진 결제 흐름의 성공·실패 분기와 일치. `CANCELED`(결제 후 취소) 등은 추후 라운드에 추가 |
| D5 | 어드민 API **scope out** | 사용자가 명시한 설계 범위에 부재 |
...
| D11 | 결제 도메인이 **포인트·PG 통합 책임자**. 주문 도메인은 결제 한 군데만 호출 | Payment record가 *지불 사실의 단일 진실 원천*. 환불·매출집계·분쟁 시 결제 record 하나로 `usedPoint + pgAmount` 분기 처리. 포인트를 결제 외부에서 차감하면 환불 record가 외부 join을 강요해 결합 발생 |
