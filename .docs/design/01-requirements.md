...
| email | String | 이메일 포맷 검증, 중복 불가 (탈퇴 계정 제외, 활성 계정만) |
...
- 브랜드 이름은 활성 브랜드 내에서 중복 불가 (등록/수정 모두 적용)
- 삭제된 브랜드의 이름은 재사용 가능 (재입점 시나리오 허용)
...
| 도메인 | 정책 | 비고 |
|--------|------|------|
| Member | Soft Delete (`deleted_at`) | 탈퇴 후 주문 이력 보존. 탈퇴 후 동일 로그인 ID / 이메일 재가입 가능 |
