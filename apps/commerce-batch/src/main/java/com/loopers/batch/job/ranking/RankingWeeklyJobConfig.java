...
public class RankingWeeklyJobConfig {
    ...
    private final WeeklyProductRankMvJpaRepository mvRepository;
    ...
    public Step rankStep(@Value("#{jobParameters['snapshotDate']}") LocalDate snapshotDate) {
        return new StepBuilder("weeklyRankStep", jobRepository)
            .<RankedRow, WeeklyProductRankMv>chunk(TOP_N, transactionManager)
            .reader(rankReader())
            .processor(row -> new WeeklyProductRankMv(snapshotDate, row.rank(), row.productId(), row.score()))
