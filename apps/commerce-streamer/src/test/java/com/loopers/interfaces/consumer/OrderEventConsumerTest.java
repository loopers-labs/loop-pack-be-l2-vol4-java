...
class OrderEventConsumerTest {
    ...
    private final OrderEventConsumer consumer = new OrderEventConsumer(new ObjectMapper(), processor);

    private ConsumerRecord<String, byte[]> rec(String value) {
        return new ConsumerRecord<>("order-events", 0, 0L, "1", value.getBytes(StandardCharsets.UTF_8));
    }

    private static final String OK =
        "{\"eventId\":\"e2\",\"type\":\"OrderPlaced\",\"orderId\":55,\"lines\":[{\"productId\":11,\"quantity\":2}],\"occurredAt\":\"t\"}";

    @DisplayName("정상 메시지는 처리하고 ack 한다.")
    @Test
    void processesAndAcks() {
        consumer.consume(List.of(rec(OK)), ack);
        verify(processor).handleOrder(any(OrderEventMessage.class));
