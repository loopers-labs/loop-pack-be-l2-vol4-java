 ...
 *   <li>{@code gaveUp} — PG가 끝내 미확정/미발견이고 데드라인까지 지나, 더 기다리지 않고 실패 확정(재고·쿠폰 원복)한 건수</li>
 *   <li>{@code stillPending} — PG도 아직 미확정(또는 거래 미발견)이지만 데드라인 전이라 다음 회차로 미룬 건수</li>
 *   <li>{@code skipped} — 거래키 없는 고아 또는 조회~확정 사이 다른 경로가 먼저 확정한(경합) 건수</li>
 * </ul>
 */
public record PaymentReconcileResult(int scanned, int paid, int failed, int gaveUp, int stillPending, int skipped) {
}
