...
class ProductTest {
    ...
    private Product product(int stock) {
        return new Product(1L, "상품", "설명", 1_000L, stock);
    }
    ...
    class Create {
        ...
        void throwsBadRequest_whenBrandIdIsNull() {
            CoreException result =
                assertThrows(CoreException.class, () -> new Product(null, "상품", "설명", 1_000L, 10));
        ...
        void throwsBadRequest_whenStockIsNegative() {
            CoreException result =
                assertThrows(CoreException.class, () -> new Product(1L, "상품", "설명", 1_000L, -1));
            assertThat(result.getErrorType()).isEqualTo(ErrorType.BAD_REQUEST);
        }
    }
