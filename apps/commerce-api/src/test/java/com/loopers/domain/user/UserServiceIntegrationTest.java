...
public class UserServiceIntegrationTest {
    ...
    class JoinUser {
        ...
        @Test
        void saveAndReturnUserModel_whenValidInfoProvided() {
            // act
            User user = userService.join(loginId, loginPassword, name, birthday, email);
        ...
        void saveEncryptedPassword_whenJoinSucceeds() {
            ...
            // assert
            assertAll(
                    () -> assertThat(user.getLoginPassword()).isNotEqualTo(loginPassword),
                    () -> assertThat(passwordEncryptor.matches(loginPassword, user.getLoginPassword())).isTrue()
            );
        }
        ...
        void throwConflictException_whenLoginIdAlreadyExists() {
            ...
            CoreException result = assertThrows(CoreException.class, () ->
                    userService.join(loginId, loginPassword, "홍길동", LocalDate.of(2000, 5, 5), "new@email.com"));
        ...
        void throwsBadRequestException_whenLoginPasswordIsInvalid(String invalidLoginPassword) {
            // act
            CoreException result = assertThrows(CoreException.class, () ->
                    userService.join(loginId, invalidLoginPassword, name, birthday, email));
        ...
        void throwsBadRequestException_whenLoginPasswordContainsBirthday(String loginPasswordWithBirthday) {
            // act
            CoreException result = assertThrows(CoreException.class, () ->
                    userService.join(loginId, loginPasswordWithBirthday, name, birthday, email));
    ...
    @DisplayName("유저 정보 조회를 할 때,")
    @Nested
    class GetUser {

        @DisplayName("올바른 로그인 ID와 비밀번호가 주어지면, 유저를 반환한다.")
        @Test
        void getUser_whenValidInfoProvided() {
            // arrange
            userService.join(loginId, loginPassword, name, birthday, email);

            // act
            User user = userService.getUser(loginId, loginPassword);
        ...
        void throwUnauthorizedException_whenLoginIdDoesNotExist() {
            // act
            CoreException result = assertThrows(CoreException.class, () ->
                    userService.getUser(loginId, loginPassword));
        ...
        void throwUnauthorizedException_whenLoginPasswordDoesNotMatch() {
            ...
            // act
            CoreException result = assertThrows(CoreException.class, () ->
                    userService.getUser(loginId, "myPassWord1234"));
    ...
    class UpdatePassword {
        ...
        void updatePassword_whenValidInfoProvided() {
            ...
            // act
            userService.changePassword(user, loginPassword, newPassword);

            // assert
            User updatedUser = userService.getUser(loginId, newPassword);
            assertAll(
                    () -> assertThat(updatedUser.getLoginId()).isEqualTo(loginId),
        ...
        @DisplayName("변경하려는 비밀번호가 현재 비밀번호와 같은 경우 BAD_REQUEST 에러를 반환한다.")
        @Test
        void throwBadRequestException_whenNewPasswordEqualsToCurrentPassword() {
            ...
            // act
            CoreException result = assertThrows(CoreException.class, () ->
                    userService.changePassword(user, loginPassword, loginPassword));
        ...
        void throwBadRequestException_whenNewPasswordDoesNotMatch(String wrongPassword) {
            ...
            // act
            CoreException result = assertThrows(CoreException.class, () ->
                    userService.changePassword(user, loginPassword, wrongPassword));
