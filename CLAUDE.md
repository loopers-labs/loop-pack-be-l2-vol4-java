...
- 도메인 서비스는 도메인 객체 유즈케이스 조율 및 트랜잭션 경계를 담당합니다.
- 애플리케이션 서비스(Facade)는 도메인 서비스 호출, 여러 도메인 서비스 유즈케이스 조율과 API/외부 인터페이스 간 변환 책임을 가집니다.
- Facade에서 예외적으로 Transaction이 필요할 수 있지만, 가급적 도메인 서비스에서 트랜잭션을 관리하는 것을 권장합니다.
- 각 Controller는 Facade의 단일 진입점으로 설계하여, API 레이어에서 비즈니스 로직이 노출되지 않도록 합니다.

## 아키텍처, 패키지 구성 전략
- 본 프로젝트는 레이어드 아키텍처, DDD 스타일의 패키지 구조를 따르며, DIP(의존성 역전 원칙)을 준수합니다.
