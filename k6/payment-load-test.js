...
const fallbackHitRate = new Rate('fallback_hit_rate');    // CB OPEN → Fallback 비율
const cbOpenCount     = new Counter('cb_open_count');     // Fallback 발동 횟수
...
export const options = {
  scenarios: {
    ...
    /**
     * 시나리오 4 — CircuitBreaker 오픈 유발 (CB Open)
     * 목적: 초당 30 요청으로 PG 실패율 50% 임계를 넘겨 CB가 열리는지 확인
     * CB 설정: sliding-window-size=10, failure-rate-threshold=50
     *   → 10건 중 6건 이상 실패하면 OPEN
     *   PG 실패율 40%이므로 운에 따라 열릴 수 있음. 빠른 속도로 반복해 유도
     * CB OPEN 판별: 응답 시간 < 50ms (PG 호출이 일어나지 않았으므로 즉시 응답)
...
/**
 * 시나리오 4: CB 오픈 유발
 * 주문 생성 → 빠른 결제 요청 반복으로 PG 실패율 50% 초과 유도
