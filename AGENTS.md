...
- 설계 결정이 필요한 구현 작업에서는 AI가 권장안, 대안, 영향 범위, 검증 방법을 제시한 뒤 사용자의 명시 승인을 받기 전까지 설계를 확정하지 않는다.
- 사용자의 승인 전에는 문서/코드 탐색, 현 상태 요약, 선택지 정리, 영향도 분석만 수행한다.
- 코드 탐색으로 얻은 판단은 확정된 설계가 아니라 관찰 또는 제안으로 표현한다.
...
- 패키지 경계, 도메인 소속, API/DTO 계약, 이벤트 종류, Kafka topic/key, Redis key, 점수 계산식, 멱등성 기준은 설계 결정으로 보고 반드시 사용자에게 먼저 묻는다.
- 설계 질문은 한 번에 한 가지 주제로 묻고, 답변을 받은 뒤 다음 질문이나 구현으로 진행한다.
